...
import {
  ...
  RadioIcon,
  Loader2,
  Instagram, // Re-added Instagram from lucide-react
} from 'lucide-react';
...
// Custom InstagramIcon component removed
    ...
    <div className="flex flex-col h-screen bg-background text-foreground">
      ...
      <div className="flex flex-col items-center justify-center text-center px-4 mt-2 mb-6">
        {displayImageUrl ? (
          <Image
            ...
            width={160}
            height={160}
            className="rounded-lg shadow-lg aspect-square object-cover mb-6"
            data-ai-hint="radio logo album art"
            key={displayImageUrl} // Added key to force re-render on image change
          />
        ) : (
          <div className="w-40 h-40 rounded-lg shadow-lg bg-muted flex items-center justify-center mb-6">
      ...
      <div className="flex justify-center items-center gap-4 mb-8 px-4">
        {currentStation.whatsappUrl && (
          <Button variant="ghost" size="icon" className="rounded-full w-12 h-12 bg-green-500 hover:bg-green-600 text-white" onClick={() => handleSocialLink(currentStation.whatsappUrl)}>
            <WhatsAppIcon />
          </Button>
        )}
        {currentStation.instagramUrl && (
          <Button variant="ghost" size="icon" className="rounded-full w-12 h-12 bg-pink-500 hover:bg-pink-600 text-white" onClick={() => handleSocialLink(currentStation.instagramUrl)}>
            <Instagram className="h-6 w-6" />
          </Button>
        )}
        {currentStation.email && (
          <Button variant="ghost" size="icon" className="rounded-full w-12 h-12 bg-yellow-500 hover:bg-yellow-600 text-white" onClick={() => handleSocialLink(`mailto:${currentStation.email}`)}>
            <MailIcon />
          </Button>
        )}
      </div>
      ...
      <div className="flex items-center justify-around p-4 mt-auto border-t border-border/20">
        <Button variant="ghost" size="icon" onClick={handleFavoriteClick} className="w-14 h-14">
          <Heart className={`h-7 w-7 ${localIsFavorite ? 'text-primary fill-current' : 'text-muted-foreground'}`} />
        </Button>
        <Button
          variant="default"
          size="icon"
        ...
        <Button variant="ghost" size="icon" className="w-14 h-14" onClick={() => navigator.share && navigator.share({ title: currentStation.name, text: `Escuchando ${currentStation.name}`, url: window.location.href })}>
